...
nav_order: 4
description: "The Legion::LLM unified schema — one format for every LLM provider, every feature, every edge case."
---

# Schema Specification

The Legion::LLM schema is the universal interchange format for all LLM interactions in LegionIO. Every request, response, error, and streaming chunk conforms to this spec regardless of which provider handles it. Provider adapters translate at the edges; everything inside the pipeline speaks one language.

{: .note }
> **Schema version: 1.0.0** — This specification uses semantic versioning. Major = breaking changes, Minor = new optional fields, Patch = documentation only.
